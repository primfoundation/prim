...
1. **Cloudflare:** Clawdflare now connects to the Eidos AGI account containing active `prims.sh` and the existing Prim Workers. The isolated `prims-hub-preview` was created, but its workers.dev/version previews remain disabled and `deployed_on` is null. Manifest registration succeeds; the static-file upload rejects fresh, unexpired scoped tokens with HTTP 401, `invalid_token`, `token verification failure`. Resolve that upload failure through an authorized client, inspect the existing preview before retrying, then deploy the exact tested module and five real Static Assets. This is no longer an installation/connection blocker. [Attempt evidence](evidence/2026-09-07-cloudflare-preview.json) records hashes, manifest, final state and unchanged existing Workers. Follow `prim-web/hub/OPERATIONS.md` and `prims-browsers/cloud/README.md`; establish operator/cost ownership and prove public clients plus a real Apple/session/container path before production cutover. Separate workers.dev hosts cannot share host-only login cookies.
